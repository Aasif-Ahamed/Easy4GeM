...
?>
        ...
        <body>
            ...
            ?>
                ...
                ?>
                    ...
                    if (empty($_FILES['itempic']['name'])) {
                        $fileName = 'N/A';
                        $insertmtdt = "INSERT INTO `masterdata`(`createdUser`,`invNo`,`rectype`,`description`, `nat_heat`, `carrat`, `owner`, `buyer`, `email`, `phone`,`dataofsell`, `noofdays`, `dateofpay`, `soldvalue`, `commis`, `shareval`,`netvalue`,`picture`, `paidval`, `balancepay`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                        $stmt = $connection->prepare($insertmtdt);
                        $stmt->bind_param('ssssssssssssssssssss', $sidn['userid'], $invoiceNo, $erot, $itemdesc, $norh, $carrat, $owner, $buyer, $emailOwners, $phoneBuyer, $dos, $nod, $paymentdate, $soldval, $commislbl, $sharelbl, $netvalLbl, $fileName, $paidVal, $balancepayable);
                        if ($stmt->execute()) {
            ...
            <table class="table table-striped table-sm" id="myTableA">
                ...
                <?php
                $mdquery = "SELECT * FROM `masterdata` WHERE `rectype`='erot' AND `createdUser`='$userdtl' ORDER BY `createtime` DESC";
